...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Offset _rescueButtonPosition = const Offset(
    20,
    100,
  ); // Will be set to right side
  ...
  Future<void> _loadDailyBriefing(String userName) async {
    ...
    try {
      ...
      if (locationData != null) {
        debugPrint(
          'Location obtained: ${locationData.cityName} (${locationData.latitude}, ${locationData.longitude})',
        );
      ...
      if (mounted) {
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text(
              'Günlük özet yüklenirken bir hata oluştu. Lütfen daha sonra tekrar deneyin.',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(context).pop(),
                  ...
                  child: GestureDetector(
                    onPanUpdate: (details) {
                      setState(() {
                        ...
                        final newPosition =
                            _rescueButtonPosition + details.delta;
                        final screenWidth = MediaQuery.of(context).size.width;
                        final screenHeight = MediaQuery.of(context).size.height;
                    ...
                    child: _buildDraggableRescueButton(
                      theme,
